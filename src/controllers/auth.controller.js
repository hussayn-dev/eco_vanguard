...
class AuthController {
  ...
  async verifyEmail(req, res, next) {
    ...
    if (!user) {
      throw new error.UnauthenticatedError(
        "Verification Failed, SignUp again",
        "user not found"
      );
    }
  ...
  async forgotPassword(req, res, next) {
    let { email } = req.body;
    if (!email)
      throw new error.NotFoundError("Please enter all fields", "empty field");
    email = email.email.toLowerCase();
    const user = await User.findOne({ email });

    if (!user)
      throw new error.NotFoundError("User does not exist", "email not found");
    let passwordResetToken = crypto.randomBytes(70).toString("hex");

    const origin = `${req.protocol}://${req.get("host")}`;

    const mail = await sendForgetPasswordEmail(
      user.fullName,
      email,
      passwordResetToken,
      origin
  ...
  async resetPassword(req, res, next) {
    ...
    if (
      ...
      throw new error.BadRequestError("Password reset failure, link expired");
  ...
  async logIn(req, res, next) {
    ...
    const user = await User.findOne({ email });
    if (!user) throw new error.BadRequestError("Email not found");
    ...
    res
      ...
      .json({ data: payload, message: "User login successfull" });
  }
