...
const {eventUpload, uploadImage} = require("../utilities/multer")
...
class EventController {
 async createEvent(req, res, next) {
    ...
    if(!name || !date ||  !about) throw new error.BadRequestError("Input fields correctly")
...
//upload event Picture
    const imageFile = await uploadImage(eventUpload, req, res)
    if (imageFile) {
    ...
    event.imageSecureUrl = uploadResponse.secure_url
     ...
     const event = new Event({name, date, about, imagePublicId, imageSecureUrl} )
     await event.save()
     res.status(StatusCodes.CREATED).json({data : event, status : true, message : "Event created successfully"})
 ...
 async getAll(req, res, next) {
    ...
    res.status(StatusCodes.CREATED).json({data : events , length : events.length, status : true, message : "successfull"})
}
 async getOne(req, res, next) {
    const event = await Event.findById(req.params.id)
    if(!event) throw new error.BadRequestError("event not found")
    res.status(StatusCodes.OK).json({data : event, status : true, message : "Successful"  })   
}
async editOne(req, res, next) {
    const event = await Event.findById(req.params.id)
    if(!event) 
       throw new error.BadRequestError("event not found")
    ...
    await event.save()
    res.status(StatusCodes.OK).json({data: event, status : true, message : "Event updated successfully "})
...
async deleteOne(req, res, next) {
    ...
    const data = {
    status : true, message : "event successfully deleted", event
   }
