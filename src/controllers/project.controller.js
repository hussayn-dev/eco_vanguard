...
const {projectUpload, uploadImage} = require("../utilities/multer")
...
// project picture
    const imageFile = await uploadImage(projectUpload, req, res)
    if (imageFile) {
    ...
    const project = new Project({name, beneficiary, type, about,  imagePublicId, imageSecureUrlx} )
 await project.save()
 res.status(StatusCodes.CREATED).json({data : project, status : true, message : "Project created successfully"})
  ...
  async getAll(req, res, next) {
    ...
    res.status(StatusCodes.CREATED).json({data : projects , length : projects.length, status : true, message : "successfull"})

}
async getOne(req, res, next) {
    const project = await Project.findById(req.params.id)
    if(!project) throw new error.BadRequestError("project not found")
    res.status(StatusCodes.OK).json({data : project, status : true, message : "Successful"  })
  }
async editOne(req, res, next) {
  ...
  await project.save()
  res.status(StatusCodes.OK).json({data: project, status : true, message : "Project updated successfully "})
...
async deleteOne(req, res, next) {
  ...
  await project.remove()   
    ...
    const data = {
    status : true, message : "project successfully deleted", project
   }
